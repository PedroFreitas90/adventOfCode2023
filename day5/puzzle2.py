...
def get_seeds(seeds_line):
    seeds_list = seeds_line.split()
    for i in range(0,len(seeds_list), 2):
        start_seed = int(seeds_list[i])
        seed_range=int(seeds_list[i+1])
        for j in range(0, seed_range):
            seeds.append(start_seed + j)
...
def calculate_minimum_location(seeds_dic): 
    locations = [seeds_dic[seed]['location'] for seed in seeds_dic]
    print(locations)
